...
ACCESS is a nested model, with a global model, ACCESS-G, at a $0.23^\circ \times 0.35^\circ$ ($\approx 25$ km) spatial resolution, with a time-step ranging from $5$ to $10$ minutes, providing boundary conditions to a $0.11^\circ$ ($\approx 12$ km) model, with a time-step ranging from $2.5$ to $5$ minutes, ACCESS-R, which covers the Australian region from $65.0^\circ$ south to $16.95^\circ$ north, and $65.0^\circ$ east to $184.57^\circ$ east. ACCESS-R also provides boundary conditions to $0.0135^\circ$ ($\approx 1.5$ km) spatial resolution, $50$ second time-step, convection permitting models, ACCESS-C, run over each Australian capital city. Fig.~XX provides a map of ACCESS-R and ACCESS-C grids. Two ACCESS model guidance datasets are available to forecasters: ACCESS-R, and ACCESS-C embedded within ACCESS-R. In this study we compare the Official forecast just to ACCESS-R. \textcolor{red}{as it is more commonly used by forecasters than ACCESS-C. (This is my memory from Darwin, but not sure in general.)}. The ECMWF model used by BoM forecasters is the high resolution 10-day forecast model (HRES), which runs at an $\approx 9$ km spatial resolution, with a 7.5 minute time step \citet{ecmwf19b}. \textcolor{red}{Need to confirm the Bureau is indeed using HRES and not just the control member of the ECMWF ensemble.}

Datasets are standardised using the same methods the Bureau uses when calculating routine verification statistics. The Official forecast dataset is at a 6 km spatial resolution, with a 3 km sub-grid covering Victoria and Tasmania. Note that when forecasters load model guidance products into GFE they are interpolated or upscaled to this resolution. Similarly, the Official forecast dataset is at UTC hourly time intervals, so just the ACCESS data at each hour UTC is considered. However, the HRES datasets the Bureau receives from ECMWF are at a three hourly time interval, so these are linearly interpolated to an hourly resolution when loaded into GFE. Thus, verification is performed on these interpolated or upscaled datasets, i.e. on the datasets that forecasters use within GFE.      
 
Australian AWS typically record wind and direction each minute. After basic quality control, 10 minute averages of each are taken at each station at each hour UTC, usually over the ten minutes leading up to each hour. When calculating verification statistics, each station is matched with the nearest 3 or 6 km grid-point in the datasets described above.         
